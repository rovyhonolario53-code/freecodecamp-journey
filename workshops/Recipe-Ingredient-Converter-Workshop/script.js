...
const updateResultsList = () => {
  ...
  for (let i = 0; i < units.length; i++) {
    if (units[i] !== unitToConvert.value) {
      resultList.innerHTML += `<li>${ingredientName.value}: ${processIngredient(Number(ingredientQuantity.value), unitToConvert.value, units[i], Number(numberOfServings.value))} ${units[i]}</li>`
    }
  }
};
